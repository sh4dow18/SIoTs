...
def on_message_received(status, msg, packet):
	...
	# If the message has the subject "/DOOR2" and the payload is "open" or "close" you can open
	# or close the door. This instruction is received from SBC 2 with motion detection.
	if msg.count("/DOOR2") == 1:
		if msg.count("open") == 1:
			digitalWrite(0, HIGH)
...
def main():
	...
	# It waits 90 seconds to allow the DHCP protocol to work properly, also it does not interfere with most MQTT packets from SBC 1 and SBC 2.
	delay(90000)
	# Connects to the Broker with "cisco3" user
	mqttclient.connect("4.2.5.2", "cisco3", "cisco3")
	delay(2000)
	# Subscribe to specific topic
	mqttclient.subscribe("/DOOR2")
